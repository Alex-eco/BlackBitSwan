...
// ✅ Раздаём фронтенд ТОЛЬКО для корня, а не всех маршрутов
...
async function performScan() {
  try {
    ...
    lastScan = {
      ...
      per_site: per,
      mood_percent: mood,
      raw_headlines: all.slice(0, 200)
    };
    console.log('🛰 Scan completed:', lastScan.timestamp, 'mood', mood);
  } catch (e) {
...
app.get('/api/contact', (req, res) =>
  res.json({ email: 'rstyleh@gmail.com', telegram: '@blackbitswan' })
);

// ✅ ВАЖНО: index.html только по не-API маршрутам
app.get('*', (req, res) => {
  res.sendFile(path.join(__dirname, '../frontend/index.html'));
});

app.listen(PORT, () => console.log(`🚀 BlackBitSwan backend running on ${PORT}`));
